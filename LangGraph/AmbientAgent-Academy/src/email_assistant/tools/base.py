...
def get_tools(tool_names: Optional[List[str]] = None, include_gmail: bool = False) -> List[BaseTool]:
    ...
    Args:
        tool_names (Optional[List[str]]): Optional list of tool names to include. If None, returns all tools.
        include_gmail (bool): Whether to include Gmail tools. Defaults to False.
        
    Returns:
        (List[BaseTool]): List of tool objects
    """
    ...
    if include_gmail:
        try:
            ...
            # If Gmail tools can't be imported, we skip them
            pass
